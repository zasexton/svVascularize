...
class ParameterPanel(QWidget):
    ...
    def _init_ui(self):
        ...
        # Mode selection
        mode_group = QGroupBox(f"{CADIcons.SETTINGS} Generation Mode")
        mode_layout = QVBoxLayout()
        mode_group.setLayout(mode_layout)

        self.mode_combo = QComboBox()
        self.mode_combo.addItem(f"{CADIcons.TREE} Single Tree")
        self.mode_combo.addItem(f"{CADIcons.FOREST} Forest (Multiple Trees)")
        ...
        # Tree parameters
        tree_params_group = QGroupBox(f"{CADIcons.SETTINGS} Tree Parameters")
        ...
        self.forest_params_group = QGroupBox(f"{CADIcons.FOREST} Forest Parameters")
        ...
        # Advanced parameters
        advanced_group = QGroupBox(f"{CADIcons.SETTINGS} Advanced Parameters")
        ...
        self.generate_btn = QPushButton(f"{CADIcons.PLAY} Generate Tree/Forest")
        self.generate_btn.clicked.connect(self._generate)
        self.generate_btn.setObjectName("primaryButton")
        self.generate_btn.setToolTip("Generate vascular tree or forest with current parameters")
        button_layout.addWidget(self.generate_btn)

        self.export_btn = QPushButton(f"{CADIcons.SAVE} Export Configuration")
    ...
    def _generate(self):
        ...
        try:
            ...
            if self.parent_gui:
                self.parent_gui.update_status(f"{CADIcons.ERROR} Generation failed")
            QMessageBox.critical(
                self,
                f"{CADIcons.ERROR} Generation Error",
    ...
    def _generate_tree(self, n_vessels, physical_clearance, convexity_tolerance, random_seed, config):
        ...
        # Update status
        if self.parent_gui:
            self.parent_gui.update_status(f"{CADIcons.PLAY} Generating tree with {n_vessels} vessels...")

        # Create progress dialog
        progress = QProgressDialog(
            f"{CADIcons.TREE} Generating tree...\nThis may take a few moments.",
            f"{CADIcons.CROSS} Cancel",
            0,
            n_vessels,
            self
        )
        ...
        # Update status
        if self.parent_gui:
            self.parent_gui.update_status(f"{CADIcons.CHECK} Tree generated successfully with {tree.n_terminals} vessels")

        QMessageBox.information(
            self,
            f"{CADIcons.CHECK} Success",
    ...
    def _generate_forest(self, n_vessels, physical_clearance, convexity_tolerance,
        ...
        # Update status
        if self.parent_gui:
            self.parent_gui.update_status(f"{CADIcons.PLAY} Generating forest with {n_vessels} total vessels...")

        # Create progress dialog
        progress = QProgressDialog(
            f"{CADIcons.FOREST} Generating forest...\nThis may take a few moments.",
            f"{CADIcons.CROSS} Cancel",
            0,
            n_vessels,
            self
        ...
        if self.parent_gui:
            self.parent_gui.update_status(
                f"{CADIcons.CHECK} Forest generated successfully with {total_vessels} vessels across {n_networks} networks"
            )
